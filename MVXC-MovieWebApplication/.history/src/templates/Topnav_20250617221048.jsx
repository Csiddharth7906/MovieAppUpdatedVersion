...
const Topnav = () => {
  ...
  return (
    <div className="w-[50] h-[8vh] relative flex justify-center items-center">
      <i className="text-zinc-400 text-3xl ri-search-line"></i>
      <input
        onChange={(e)=> setquery(e.target.value)}
        value={query}
        className="w-[50%] p-5 text-xl outline-none border-none bg-red-100 text-zinc-400  mx-10"
        type="text"
        placeholder="search anything"
      />
      {query.length >0 &&(
        
        <i onClick={()=>{setquery("")}} className="absolute right-0 text-zinc-400 text-3xl ri-close-fill"></i>
      )}
