import React from "react";
import { Link } from "react-router-dom";

const Sidenav = () => {
  return (
    <div className="w-[20%] h-full  border-r-2 border-zinc-400 p-10">
      <h1 className=" text-2xl text-white font-bold ">
        <i classname="text-[#D2042D] ri-tv-fill mr-2"></i>
        <span className="text-2xl ">MVXC</span>
      </h1>
      <nav className="flex flex-col text-zinc-400 text-xl gap-3">
        <h1 className="text-white font-semibold mt-10 mb-5">New Feeds</h1>
        <Link className="hover:bg-[#D2042D] hover:text-white rounded-lg duration-300 p-3">
          <i classname="mr-2 ri-fire-fill"></i>Trending
        </Link>
        <Link className="hover:bg-[#D2042D] hover:text-white rounded-lg duration-300 p-3">
          <i classname="mr-2 ri-bard-fill"></i> Popular
        </Link>
        <Link className="hover:bg-[#D2042D] hover:text-white rounded-lg duration-300 p-3">
          <i classname="mr-2 ri-clapperboard-fill"></i> Movies
        </Link>
        <Link className="hover:bg-[#D2042D] hover:text-white rounded-lg duration-300 p-3">
          <i classname="mr-2 ri-tv-2-fill"></i> Tv Shows
        </Link>
        <Link className="hover:bg-[#D2042D] hover:text-white rounded-lg duration-300 p-3">
          <i classname="mr-2 ri-user-smile-fill"></i> People
        </Link>
      </nav>
      <hr className="border-none h-[1px] bg-zinc-400 mt-3 mb-0" />
      <nav className="flex flex-col text-zinc-400 text-xl gap-3">
        <h1 className="text-white font-semibold text-xl mt-10 mb-5">
          Website Information
        </h1>
        <Link className="hover:bg-[#D2042D] hover:text-white rounded-lg duration-300 p-2">
          <i classname="mr-2 ri-information-2-fill"></i>About
        </Link>
        <Link className="hover:bg-[#D2042D] hover:text-white rounded-lg duration-300 p-2">
          <i classname="mr-2 ri-contacts-fill"></i>Contact
        </Link>
      </nav>
    </div>
  );
};

export default Sidenav;
